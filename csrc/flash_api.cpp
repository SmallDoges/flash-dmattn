...
mha_bwd(
    ...
    if (num_heads_k != num_heads) {
        ...
        at::sum_out(dbias, at::reshape(dbias_expanded, {batch_size, num_heads_k, num_heads / num_heads_k, seqlen_q, seqlen_k}), {2});
    }
...
//                 "FlashDynamicMaskAttention only support fp16 and bf16 data type");
//     TORCH_CHECK(k.dtype() == q_dtype, "query and key must have the same dtype");
//     TORCH_CHECK(v.dtype() == q_dtype, "query and value must have the same dtype");
//     TORCH_CHECK(out.dtype() == q_dtype, "query and out must have the same dtype");
//     TORCH_CHECK(dout.dtype() == q_dtype, "query and dout must have the same dtype");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("bwd", &FLASH_NAMESPACE::bwd, "Backward pass");
    // m.def("varlen_bwd", &FLASH_NAMESPACE::varlen_bwd, "Backward pass with variable length");
}
