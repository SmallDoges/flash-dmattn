...
template<typename Kernel_traits, bool Is_causal, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Split, bool Append_KV, typename Params>
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    Tensor tVsV = gmem_thr_copy_QKV.partition_D(sV);
    Tensor tZOHgZOH = gmem_thr_copy_ZOH.partition_S(gZOH);      // (ZOHCPY, ZOHCPY_M, ZOHCPY_N)
    Tensor tZOHsZOH = gmem_thr_copy_ZOH.partition_D(sZOH);
    Tensor tAMgAM = gmem_thr_copy_AM.partition_S(gActiveMask);  // (AMCPY, AMCPY_M, AMCPY_N)
    Tensor tAMsAM = gmem_thr_copy_AM.partition_D(sActiveMask);
    ...
    if (!Append_KV || params.rotary_dim == 0) {
        ...
        const index_t row_offset_cossin = (binfo.seqlen_k_cache + (params.leftpad_k == nullptr ? 0 : params.leftpad_k[bidb]) + (Is_causal ? m_block * kBlockM : 0)) * (params.rotary_dim / 2);
...
inline __device__ void compute_attn_splitkv(const Params &params) {
    ...
    FLASH_NAMESPACE::compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_even_MN, Is_even_K, Is_softcap, Split, Append_KV>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
}
